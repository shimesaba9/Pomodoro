...
import { DEFAULT_TIME_SETTINGS, MODE_CONFIG } from './constants.js';
...
const App = () => {
    const [timeSettings, setTimeSettings] = useState(getInitialTimeSettings);
    const [isSettingsOpen, setIsSettingsOpen] = useState(false);

    const [mode, setMode] = useState(TimerMode.POMODORO);
    const [timeRemaining, setTimeRemaining] = useState(timeSettings[mode]);
    const [isActive, setIsActive] = useState(false);
    const [pomodoroCount, setPomodoroCount] = useState(getInitialPomodoroCount());
